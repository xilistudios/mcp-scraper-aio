...
describe('WebScraperMCPServer', () => {
  ...
  beforeEach(() => {
    ...
    (
      BrowserManager as jest.MockedClass<typeof BrowserManager>
    ).mockImplementation(() => mockBrowserManager);

    // Mock WebsiteAnalyzer
    mockAnalyzer = {} as jest.Mocked<WebsiteAnalyzer>;
    (
      WebsiteAnalyzer as jest.MockedClass<typeof WebsiteAnalyzer>
    ...
    (
      MCPToolHandlers as jest.MockedClass<typeof MCPToolHandlers>
    ).mockImplementation(() => mockHandlers);
  ...
  describe('Constructor', () => {
    it('should create an instance with all dependencies', () => {
      ...
      expect(WebsiteAnalyzer).toHaveBeenCalledWith(
        mockBrowserManager,
        expect.anything()
      );
      expect(MCPToolHandlers).toHaveBeenCalledWith(
        mockAnalyzer,
        expect.anything()
      );
            ...
            inputSchema: {
              ...
              properties: {
                ...
                requestId: {
                  type: 'string',
                  description:
                    'The unique ID of the request to get details for',
                },
              },
  ...
  describe('Call Tool Handler', () => {
    ...
    it('should handle analyze_website_requests tool', async () => {
      ...
      expect(mockHandlers.handleAnalyzeWebsite).toHaveBeenCalledWith({
    ...
    it('should handle get_requests_by_domain tool', async () => {
      const mockResult = {
        content: [{ type: 'text', text: 'domain requests' }],
      };
    ...
    it('should handle get_request_summary tool', async () => {
      ...
      expect(mockHandlers.handleGetRequestSummary).toHaveBeenCalledWith(
        'https://example.com'
      );
      expect(result).toEqual(mockResult);
    });
